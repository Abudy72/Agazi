...
export async function getPlayerStatsByDivision(divisionId: number): Promise<PlayerStats[]> {
    ...
    .select({
        ...
        // Win/Loss stats
        wins: sql<number>`COUNT(CASE WHEN ${matchData.team} = ${matches.winning_team} THEN 1 END)`.as("wins"),
        losses: sql<number>`COUNT(CASE WHEN ${matchData.team} != ${matches.winning_team} THEN 1 END)`.as("losses"),
